...
class QFlatGroupingTreeModel(QAbstractItemModel):
    ...
    def __init__(
        ...
        self._flat_group_arrays: Dict[str, List[Array]] = {}
    ...
    def index(
        ...
        if 0 <= column < len(self.COLUMNS):
            if parent.isValid():
                flat_group = parent.internalPointer()
                ...
                return self.createIndex(row, column, object=self._flat_groups[row])
        return QModelIndex()
                ...
                if index.column() == self.COLUMNS.NAME:
                    if role == Qt.ItemDataRole.EditRole:
                        ...
                        if value not in self._flat_groups:
                            for array in self._flat_group_arrays[flat_group]:
    ...
    def dropMimeData(
        ...
        if (
            ...
            while len(indices_stacks) > 0:
                ...
                self._pending_flat_group_changes[array] = flat_group
            return True
        return False
    ...
    def removeRows(
        ...
        if parent.isValid():
            old_flat_group = parent.internalPointer()
            assert isinstance(old_flat_group, str)
            arrays = self._flat_group_arrays[old_flat_group]
    ...
    def _on_array_visible_event(self, event: Event) -> None:
        ...
        if (
            ...
            self.dataChanged.emit(flat_group_index, flat_group_index)
            array_index = self.create_array_index(array, column=self.COLUMNS.VISIBLE)
            self.dataChanged.emit(array_index, array_index)
    ...
    @property
    def flat_group_arrays(self) -> Mapping[str, Sequence[Array]]:
