...
from ._array_group_table_model import QArrayGroupTableModel


# TODO styling (checkboxes)
# TODO make array groupings user-editable --> QTreeView
class QArrayGroupTableView(QTableView):
    def __init__(
        self,
        ...
        self._model = QArrayGroupTableModel(
            controller, array_grouping=array_grouping, close_callback=close_callback
        )
        self._updating_selection = False
        self._updating_current_arrays_selection = False
        self._proxy_model = QSortFilterProxyModel()
        self._proxy_model.setSourceModel(self._model)
        self._proxy_model.sort(QArrayGroupTableModel.COLUMNS.NAME)
        self.setModel(self._proxy_model)
        self.verticalHeader().hide()
        self.horizontalHeader().hide()
        self.horizontalHeader().setStretchLastSection(False)
        self.horizontalHeader().setSectionResizeMode(
            QArrayGroupTableModel.COLUMNS.NAME, QHeaderView.ResizeMode.Stretch
        )
        self.horizontalHeader().setSectionResizeMode(
            QArrayGroupTableModel.COLUMNS.LOADED,
            QHeaderView.ResizeMode.ResizeToContents,
        )
        self.horizontalHeader().setSectionResizeMode(
            QArrayGroupTableModel.COLUMNS.VISIBLE,
            QHeaderView.ResizeMode.ResizeToContents,
        )
    ...
    def _on_selection_changed(
        ...
        if not self._updating_selection:
            ...
            for proxy_index in self.selectionModel().selectedRows():
                ...
                array_group = self._model.array_groups[index.row()]
                array_group_arrays = self._model.array_group_arrays[array_group]
                new_current_arrays_selection.update(array_group_arrays)
